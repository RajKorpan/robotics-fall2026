...
def render(st):
    ...
    if st.button("Generate final manifest",type="primary",disabled=bool(missing) or not 200<=words<=300 or not reflection_ready):write_final_reflection(st);path=write_manifest(st);st.success(f"Submission ready at {path.parent}")
