...
def save_mission(mission, evidence, responses, uploads=()):
    target = submission_root() / mission; target.mkdir(parents=True, exist_ok=True); payload = {"schema_version": 1, "lab_id": LAB.id, "mission_id": mission, "saved_at": datetime.now(timezone.utc).isoformat(), "evidence": evidence}
    (target / "submission.json").write_text(json.dumps(payload, indent=2), encoding="utf-8"); prefix = mission + "."; lines = [f"# {mission.replace('_', ' ').title()}", ""]
...
def write_manifest(st):
    root = submission_root(); files = sorted(str(path.relative_to(root)).replace("\\", "/") for path in root.rglob("*") if path.is_file() and path.name != "manifest.json"); payload = {"schema_version": 1, "lab_id": LAB.id, "generated_at": datetime.now(timezone.utc).isoformat(), "student": dict(st.session_state["student"]), "completed_missions": list(st.session_state["completed_missions"]), "files": files}
